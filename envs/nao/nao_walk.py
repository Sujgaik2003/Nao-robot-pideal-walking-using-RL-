...
class JvrcWalkEnv(mujoco_env.MujocoEnv):
    def __init__(self):
        ...
        path_to_xml_out = '/tmp/mjcf-export/jvrc_walk/nao.xml'
      ...
      #'L_HIP_P', 'L_HIP_R', 'L_HIP_Y', 'L_KNEE', #'L_ANKLE_R', 'L_ANKLE_P'
        ...
        # set up interface
        self.interface = robot_interface.RobotInterface(self.model, self.data, 'RAnklePitch', 'LAnklePitch')
        # set up task
        self.task = walking_task.WalkingTask(client=self.interface,
                                               dt=control_dt,
                                               neutral_foot_orient=np.array([1, 0, 0, 0]),
                                               root_body='Torso',
                                               lfoot_body='LAnklePitch',
                                               rfoot_body='RAnklePitch',
                                               head_body='HeadPitch',
        )
        # set goal height
